...
from .forms import RegisterForm, ReviewForm, PersonForm
from .models import CarDealer, CarMake, CarModel, DealerReview, Person
...
def get_dealer_details(request):
    ...
    if request.method == "GET":
        ...
        dealer_response = requests.get(
            url=str(dealership_url), auth=HTTPBasicAuth("apikey", str(dealership_key))
        )
        rev_response = requests.get(
            url=str(reviews_url), auth=HTTPBasicAuth("apikey", str(dealership_key))
        )
        ...
        for review in reviews:
            try:
                if int(review["doc"]["id"]) == int(id):
                    ...
                    review["doc"]["sentiment_emoticon"] = get_emoticon_from_sentiment(
                        sentiment_dict
                    )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            response = requests.post(
                ...
                headers=headers,
                data=json_to_send,
                auth=HTTPBasicAuth("apikey", str(dealership_key)),
            )

    cars = CarModel.objects.all()
    return render(
        request,
        "djangoapp/addreview.html",
        {
            "emptyform": form_names,
